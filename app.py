...
def get_plans():
    ...
    try:
        ...
        print(f"Error in get_plans: {str(e)}")
    ...
    try:
        ...
        if data.get('photo'):
            try:
                ...
                photo_url = data['photo']  # Fallback to original URL
...
def update_plan(plan_id):
    ...
    try:
        ...
        if data.get('photo') and isinstance(data['photo'], str) and data['photo'].startswith('data:image'):
            try:
                upload_result = cloudinary.uploader.upload(
                    data['photo'],
                    folder=f"plans/{session.get('session_id', 'temp')}",
                    public_id=str(plan_id),
                    overwrite=True
                )
                photo_url = upload_result['secure_url']
...
def delete_plan(plan_id):
    ...
    try:
        ...
        for pid in all_ids:
            try:
                cloudinary.uploader.destroy(f"plans/{session.get('session_id', 'temp')}/{pid}")
            except:
                pass
...
def get_plan_content(plan_id):
    ...
    try:
        ...
        print(f"Error in get_plan_content: {str(e)}")
        return jsonify({'success': False, 'error': str(e)}), 500
    ...
    try:
        ...
        for item in content:
            if item['id'] == content_id:
                ...
                if item['type'] == 'photo' and data['data'].get('images'):
                    for img in data['data']['images']:
                        if img.get('preview', '').startswith('data:image'):
                            try:
                                upload_result = cloudinary.uploader.upload(
        ...
        print(f"Error in update_plan_content: {str(e)}")
        return jsonify({'success': False, 'error': str(e)}), 500
...
def delete_plan_content(plan_id, content_id):
    ...
    try:
        ...
        print(f"Error in delete_plan_content: {str(e)}")
        return jsonify({'success': False, 'error': str(e)}), 500
