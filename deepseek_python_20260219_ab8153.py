...
def create_tables():
    ...
    try:
        # Note: In Supabase, you need to create tables via SQL editor first
        # This function just checks connectivity
        supabase.table('plans').select('*').limit(1).execute()
        print("✅ Database connected successfully")
    except Exception as e:
        print(f"⚠️  Make sure tables exist in Supabase: {e}")

def init_db():
    """Initialize database tables - Run this once via SQL in Supabase"""
    sql = """
    -- Plans table (without metadata column)
    ...
    -- Create policy for user isolation (if you add user authentication later)
    CREATE POLICY "Users can access their own plans" 
    ON plans FOR ALL 
    USING (auth.uid()::text = user_id);
    """
    print("Run this SQL in Supabase SQL editor once:")
    print(sql)
